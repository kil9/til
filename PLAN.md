...
- **T-10 인덱스 최신순 정렬(클라이언트 JS)** — 루트 `index.html` 갤러리 카드를 최신이 위로 오도록 정렬한다. 각 카드에 `data-date`(예: `2026-07-11`)를 부여하고, 페이지 로드 시 JS 가 날짜 내림차순으로 카드를 재배치한다. 빌드 단계 없이 단일 파일 인라인 스크립트로 유지한다. (현재는 정적 HTML 삽입 순서로 수동 정렬 — `index.html:122-153`.)
