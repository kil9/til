...
페이지 3건 퍼블리시됨. 루트 갤러리는 `data-date` 최신순 정렬(T-10) + 이번달 외 카드 축약·연/월 아카이브(T-11) + 주제 필터 칩(T-14) 적용, AI요약은 카드가 아닌 서브헤딩 스타일 텍스트 블록으로 표시(T-17), nuc14 에 일 1회 AI요약 잡 가동 중(T-12, cron 09:00 KST). 남은 태스크는 T-15·T-16·T-18·T-19·T-20 이며, 그 외에는 새 `/publish-pages` 요청이 오면 AGENTS.md 런북대로 페이지를 추가한다.
