...
페이지 3건 퍼블리시됨. 루트 갤러리는 `data-date`(날짜+시각, `YYYY-MM-DDTHH:MM`) 최신순 정렬(T-10, 카드에 시각까지 표시) + 이번달 외 카드 축약·연/월 아카이브(T-11, `/til-archive/` 는 구 til repo 마지막 업데이트 `2026-04-02` 기준으로 아카이브 하단 배치) + 주제 필터 칩(T-14) 적용, AI요약은 히어로(구 lede 자리)에 작은 라벨 + 안내 톤 요약문으로 표시(T-17, 페이지 목록은 제거하고 아래 갤러리 카드로 대체), nuc14 에 일 1회 AI요약 잡 가동 중(T-12, cron 09:00 KST, 모델 sonnet — `inject.py` 템플릿은 index.html 히어로의 AI-SUMMARY 블록과 반드시 일치시켜야 다음 cron 이 되돌리지 않는다). 사이트 구조는 SSG 미도입·현행 자체 완결형 HTML 방식 유지로 결정(T-16). 사이트 디자인 방향 검토(T-19)는 제안 전부 폐기로 종결됐으나(검토 내역은 I-5), 이후 T-23 에서 극한 미니멀 디자인(무채색+차가운 블루 액센트, 단일 리스트)을 루트 갤러리에 적용하고 AGENTS.md 런북에 신규 페이지 공통 셸 템플릿을 확정했다(2026-07-12). 저장소는 2026-07-11 `kil9/docs` 에서 `kil9/til` 로 리네임 완료(T-20 권고 → T-21 실행: 기존 til 노트는 `/til-archive/` 로 흡수, 원 저장소 삭제, 라이브 URL 은 `https://kil9.github.io/til/` 로 변경). `2026-07-plan-pipeline` 글에는 beads 미채택 근거 섹션(§9)이 포함돼 있다(T-22). 남은 태스크는 T-15 이며(T-18 은 T-23 의 단일 리스트 확정으로 폐기 종결), 그 외에는 새 `/publish-pages` 요청이 오면 AGENTS.md 런북대로 페이지를 추가한다.
