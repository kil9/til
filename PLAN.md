...
> **아카이브 (2026-07-13)**: 이 파일은 backlog 백엔드로 전환되어 더 이상 갱신하지 않는다.
...
페이지 9건 퍼블리시됨(matsuri/luna/plan-pipeline/ai-capex-hedge/claude-vs-gpt56-computer-use/citrini-2028-gic/hashimoto-oss-philosophy/ghostty-vs-windows-terminal/backlog-md-vs-plan-md + `/til-archive/` 아카이브, 갤러리 카드 10장). 루트 갤러리는 `data-date`(날짜+시각, `YYYY-MM-DDTHH:MM`) 최신순 정렬(T-10, 카드에 시각까지 표시) + 이번달 외 카드 축약·연/월 아카이브(T-11, `/til-archive/` 는 구 til repo 마지막 업데이트 `2026-04-02` 기준으로 아카이브 하단 배치) + 주제 필터 칩(T-14) + 검색 입력(T-26, `?q=` 딥링크·칩과 AND 결합) 적용, AI요약은 히어로(구 lede 자리)에 작은 라벨 + 안내 톤 요약문으로 표시(T-17, 페이지 목록은 제거하고 아래 갤러리 카드로 대체), nuc14 에 일 1회 AI요약 잡 가동 중(T-12, cron 09:00 KST, 모델 sonnet — `inject.py` 템플릿은 index.html 히어로의 AI-SUMMARY 블록과 반드시 일치시켜야 다음 cron 이 되돌리지 않는다). 사이트 구조는 SSG 미도입·현행 자체 완결형 HTML 방식 유지로 결정(T-16). 사이트 디자인 방향 검토(T-19)는 제안 전부 폐기로 종결됐으나(검토 내역은 I-5), 이후 T-23 에서 극한 미니멀 디자인(무채색+차가운 블루 액센트, 단일 리스트)을 루트 갤러리에 적용하고 AGENTS.md 런북에 신규 페이지 공통 셸 템플릿을 확정했다(2026-07-12). 저장소는 2026-07-11 `kil9/docs` 에서 `kil9/til` 로 리네임 완료(T-20 권고 → T-21 실행: 기존 til 노트는 `/til-archive/` 로 흡수, 원 저장소 삭제, 라이브 URL 은 `https://kil9.github.io/til/` 로 변경). `2026-07-plan-pipeline` 글에는 beads 미채택 근거 섹션(§9)이 포함돼 있다(T-22). 방문 지표는 Cloudflare Web Analytics beacon 을 전 페이지에 삽입해 수집 중(T-15, CF 대시보드에서 확인, 신규 페이지는 런북 템플릿에 포함). 남은 태스크는 없으며(T-18 은 T-23 의 단일 리스트 확정으로 폐기 종결), 새 `/publish-pages` 요청이 오면 AGENTS.md 런북대로 페이지를 추가한다.
...
- **T-22 2026-07-plan-pipeline 글에 "beads를 채택하지 않은 이유" 섹션 추가 (2026-07-12 완료)** — beads(gastownhall/beads, `bd` CLI·임베디드 Dolt DB 기반 AI 에이전트용 의존 그래프 이슈 트래커)를 조사해 §9 "대안 검토 — beads를 채택하지 않은 이유" 섹션을 추가(치트시트는 §10으로 재번호, TOC 갱신). 논지: `bd ready` 쿼리·컨텍스트 절약·원자적 클레임·무충돌 해시 ID 등 강점을 인정하되, ① 솔로 1\~2 세션·선형 백로그 규모에선 그래프 쿼리가 풀 병목이 없고(/next-plan 으로 충분) ② 파일 비대화는 /cleanup-plan 압축, 병합 충돌은 구역 분리(§7)로 이미 상쇄 ③ 런타임 의존(bd+Dolt)·진실원본의 DB 이전(GitHub 웹 열람성 상실)·마크다운 전제 스킬 전면 재작성·산문성(현재 상태·디자인 결정) 상실이 즉발 비용. 재평가 트리거(병렬 에이전트 증가로 원자적 클레임 필요, 의존 그래프 심화) 명시. 태그 밸런스·앵커 전수 검사 + 헤드리스 Firefox 라이트/다크 렌더 확인.
...
- **T-23 frontend-design 스킬로 사이트 디자인 적용 (2026-07-12 완료)** — 사용자와 단계별 체크인(방향→팔레트→레이아웃→렌더 확인)으로 진행. 확정 디자인: **극한 미니멀리즘** — 무채색(순백/근흑 쿨 뉴트럴) + 차가운 블루 액센트(`#1A5FC8`/다크 `#82B1F0`), 카드 껍데기(배경·테두리·그림자·라운드) 전부 제거, 날짜(모노)+주제를 왼쪽 거터로 두는 **단일 리스트**(본문 폭 720px, 기본 16px), 필터 칩은 텍스트 버튼(활성=블루 밑줄), `.path` 표시 제거, eyebrow 제거. 마크업·클래스·`data-*`·인라인 JS(T-10/T-11/T-14)와 `.ai-summary` 구조는 그대로 유지해 **nuc14 `inject.py` 는 무수정**. AGENTS.md 런북 §2 에 신규 페이지 공통 셸 템플릿(동일 토큰) 확정. 검증: 헤드리스 Firefox 라이트/다크/모바일(400px) 렌더 + tailscale funnel 로 사용자 실기기 확인 후 승인. 1차 3안 제안(리소 진/타이포 스펙시멘/현행 정제)은 사용자가 전부 반려하고 미니멀 방향을 직접 지정.
- **T-18 갤러리 다단 칼럼 레이아웃 (2026-07-12 폐기 종결)** — T-23 레이아웃 체크인에서 "반응형 2열(T-18 의도)" 안을 제시했으나 사용자가 **단일 리스트**를 선택해 대체됨. 정보 밀도는 카드→리스트 전환(항목당 세로 공간 축소)으로 확보. 항목 수가 크게 늘어 단일 리스트가 길어지면 재평가.
...
- **T-26 갤러리 검색 (I-2 승격, 2026-07-12 완료)** — 루트 `index.html` 에 클라이언트 사이드 검색 입력 추가(칩 아래, T-23 미니멀 룩: 밑줄 입력·포커스 시 액센트). 대상: 카드 텍스트 전체(제목·설명·태그·경로)+`href` 슬러그+`data-topic`, 대소문자 무시 부분 일치·공백 구분 다중 토큰 AND. 주제 칩(T-14)과 AND 결합(기존 `applyFilter` 를 `state {topic,q}` + `update()` 로 재구성, 칩 2개 미만이어도 검색은 동작), 아카이브(T-11) 그룹 연동(빈 그룹·빈 아카이브 숨김), `?q=` 딥링크(`?topic=` 과 조합 가능), 0건 시 안내 문구. `.ai-summary` 마커 불변(nuc14 `inject.py` 무수정), 외부 라이브러리 없음. 검증: geckodriver(WebDriver API 직접 호출) 헤드리스 Firefox 로 9케이스(기본/딥링크/0건/칩+검색 조합/실타이핑+클리어/대소문자/다중 토큰/아카이브 연동/검색 후 칩 클릭) 전부 통과 + 라이트/다크 스크린샷 + push 후 라이브 확인.
- **T-27 시각 자료 지침 + codex 삽화 파이프라인 (2026-07-13 완료)** — 런북 §2-1 신설: 표·인라인 SVG 차트·인포그래픽 보강을 기본 원칙으로, 개념 은유·분위기 환기용 래스터 삽화는 codex CLI `image_generation`(stable feature) 위임으로 확정. 삽화는 1440px WebP(q75, 약 70-100KB) 압축 후 base64 data URI 임베드(사이드카 파일 금지), `figure.illust`+`figcaption` 출처 표기+다크모드 `brightness(0.9)`. kil9conf `/publish-til` 스킬 §2-2 에 codex exec 레시피(프롬프트에 저장 절대경로·이미지 내 텍스트 금지 명시, 결과 Read 눈검사) 추가. 첫 적용: `citrini-2028-gic/` 에 '지능 대체 나선' 에디토리얼 일러스트(1536x1024 생성 → 75KB WebP) 히어로 삽입.
...
- **I-2 갤러리 검색** — → T-26 편입 (2026-07-12). 페이지가 많아지면 루트 갤러리에 검색 추가. (태그 필터는 T-14, 연도 구분은 T-11 로 구현됨.)
