...
- **T-23 (단독·대화형) frontend-design 스킬로 사이트 디자인 적용** — `frontend-design` 스킬(공식 플러그인, 설치 완료)을 써서 사이트 비주얼을 실제로 새로 디자인·적용한다. **범위: 루트 `index.html` 갤러리 + 신규 페이지용 공통 셸/템플릿까지** (기존 콘텐츠 페이지 matsuri/luna/plan-pipeline·til-archive 는 자체 완결형이므로 이번엔 건드리지 않는다). **진행 방식: 사용자와 상담하며 단계별 확인**(팔레트·타이포·레이아웃 등 주요 결정 지점마다 체크인, 한 번에 자율 완주하지 않는다). 단독 진행(팀/parallel-plan 으로 나누지 않음). 참고: T-19 는 "제안만 하고 현행 룩 유지"로 종결됐으나, 이 태스크는 사용자 요청으로 frontend-design 을 직접 써서 실제 적용까지 가는 별개 작업이다(T-19 결론을 갱신). 루트 index.html 은 AI요약(T-17)·정렬/축약/필터(T-10/T-11/T-14) 스크립트, 향후 분석 beacon(T-15)과 공존하므로 `<!-- AI-SUMMARY -->` 마커·`data-*`·인라인 JS 를 깨지 않도록 맞추고, nuc14 `inject.py` 템플릿과 히어로 마크업 정합을 유지한다. 완료 조건: 새 디자인이 라이트/다크·자체 완결·기존 기능 유지한 채 루트+공통 템플릿에 반영, 커밋.
