...
- 루트 `index.html`: 갤러리 카드를 **최신이 위로** 추가하고 `Published · N` 카운트를 증가시킨다.
  ...
  - 사이드바 월별 목차와 `/p/archive/`(전체 목록 페이지)는 루트 카드를 진실원본 삼아 JS 로 자동 생성된다(전자는 인라인 스크립트, 후자는 루트 index.html 을 fetch). 카드 추가·카운트 증가 외에 따로 갱신할 것이 없다. 격자 커버는 기본이 주제 타일(색조+아이콘, 새 주제는 해시 색·기본 아이콘 폴백)이고, 카드에 `data-thumb`(소형 이미지 data URI)을 넣으면 그 이미지가 커버로 쓰인다(선택 훅).
