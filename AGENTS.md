...
**기본 세트는 낙서풍 18종 `backlog/assets/liv/stickers-doodle/` 다**(TASK-61). 떨리는 잉크 선에 힘 빠진 실루엣이고, 크롭이 포즈마다 다르다(상반신 13·허벅지 3·전신 2 — 하반신은 그 컷이 필요로 할 때만 그린다). 복장은 포즈마다 하나씩 배분돼 OL 7종·후디 11종이다. 구 세트 `stickers/`(전신 스탠딩·매끈한 마스코트 화풍)는 폐기하지 않고 보존하되 신규 페이지에는 쓰지 않는다 — 슬러그가 동일해 그대로 갈아끼울 수 있다. 슬러그·쓰임새 표와 규격 근거는 doc-3 §4-6, 신규 세트의 차이는 §4-7 이 정본이다.
