...
- **PWA 매니페스트·아이콘은 단일 파일 원칙의 의도적 예외다**(TASK-119). 접수창구(`p/submit-<토큰>/`)가 안드로이드 공유 시트에 뜨려면 설치 가능한 PWA 여야 하고, 웹 앱 매니페스트는 `data:` URI 로 선언할 수 없으며(`<link rel="manifest">` 가 받아 가는 실제 파일이 필요하다) 매니페스트 안의 `icons[].src` 도 크롬이 설치 시 내려받는 실 URL 이어야 한다. 그래서 `manifest.webmanifest` 와 `icon-192.png`·`icon-512.png`·`icon-maskable-512.png` 를 그 페이지 디렉터리에 사이드카로 둔다. `p/archive/thumbs/`·`og/` 와 같은 결의 예외이며, 근거는 §4 의 OG 이미지 항목과 동일하다(남의 프로그램이 URL 로 읽어 가기 때문이다). 아이콘은 **PNG** 다 — 매니페스트 아이콘은 크롬 외 런처·공유 시트 구현이 읽어 가므로 WebP 지원을 전제하지 않는다. 이 예외는 PWA 를 다는 페이지에만 적용되고, 일반 아티클의 삽화·스티커는 종전대로 사이드카 금지다.
- artifact 원본 스타일을 유지하는 페이지라도 **favicon·OG 메타(위 템플릿의 `<link rel="icon">`\~`twitter:card` 블록, 제목·설명·slug 치환)와 beacon 은 반드시 넣는다**(T-24). **OG 이미지(`og:image`)는 2026-07-26 TASK-100 에서 도입했다 — T-24 의 "쓰지 않는다"는 뒤집혔다.** 당시 배제 이유는 `og:image` 가 `data:` URI 를 못 읽어(크롤러가 받아 갈 실제 URL 이 필요하다) 무예외 단일 파일 원칙과 충돌한다는 것이었고, 그 충돌은 `p/archive/thumbs/` 사이드카 예외로 이미 해소돼 있었다. 실제 손해는 Slack·카카오톡·X 공유가 전부 회색 텍스트 블록으로 뜬다는 것이었다. 메타는 손으로 넣지 않고 `backlog/assets/relink-pages.py` 가 마커 구간(`<!-- PAGEOG:START -->`)에 박으므로 신규 페이지는 §4 의 재생성 단계만 지키면 된다. 이미지는 **JPEG** 다(다른 자산은 전부 WebP 지만, 남의 크롤러가 읽어 가므로 카카오톡·X 의 WebP 지원 불확실성을 감수하지 않는다). 파비콘은 전 페이지 공통 **리브 원형 아이콘**(64px WebP, 얼굴 크롭 + 원형 마스크)이며 페이지별 커스텀 파비콘을 만들지 않는다 — base64 데이터는 루트 `index.html` 의 것을 복사한다.
...
- **위치 제한 없음**: 도입부(작성 경위), 본문 중간(곁가지 감상·실수 고백), 마무리(소감·다음 안내) 중 자연스러운 곳에 넣는다.
...
- **드물수록 효과가 크다(TASK-11)**: 한 번에 1-2문장, 페이지 전체로도 서너 번이면 충분하다. 한 가지 캐릭터성(특히 귀찮음·나른함)을 문단마다 반복하면 금방 질린다. 삽화가 이미 나른함을 보여주므로 글에서 되풀이할 필요가 없다. 애매하면 뺀다.
- **톤**: 존댓말, 사이트 주인 호칭은 "관리자님". 나른하되 담백하게. 과장·이모지·느낌표를 남발하지 않는다(느낌표는 페이지당 1회 이하가 기본).
- **호칭(TASK-20)**: 풀네임 "리브 투데이"는 소개 페이지(`/p/liv-today/`)의 자기소개부(인사말·이름 유래)와 공개 설정화 페이지(`/p/liv-today/sheet/`)에서만 노출한다. 그 외 모든 곳(본문·바이라인·meta description·삽화 alt·루트 인덱스)에서는 "리브"로 쓴다. 저장소 문서(README·AGENTS.md·doc-3)는 캐릭터를 정의하는 문서라 풀네임을 유지한다.
- **"~한 자리" 금지**: 위치를 뜻하는 명사 "자리"의 반복은 어색하다 — "곳"·"문단"·"번"으로 바꾸거나 서술형으로 푼다(관용 표현 "자리를 맞추다" 등은 예외). 2026-09-02 사용자 피드백, 정본은 doc-3 §3-1.
...
- 루트 `index.html` 은 본문(`.feed`, 카드 목록)과 우측 사이드바(`aside.side`, 260px) 2칼럼이다. 사이드바 위쪽이 리브가 상설로 노출되는 `a.keeper` 블록(아바타·이름·한 줄 소개 전체가 `/p/liv-today/` 로 가는 링크), 그 아래가 사람 관리자 `.human` 블록(kil9, 링크 없음 — 아바타·이름·한 줄 소개 스타일은 `.keeper` 와 공유한다)이며, 그 아래가 '리브의 코멘트'(`<!-- AI-SUMMARY:START -->`\~`END`)다(TASK-13 에서 소개가 먼저 오도록 재배치). 구분선(`border-top`)은 `.ai-summary` 앞에만 두고 소개 두 블록 사이는 여백만 준다 — `a.keeper` 는 사이드바 최상단이라 `margin-top`·`padding-top`·`border-top` 이 없다. `.human` 아바타는 24px 도트 원본이라 `image-rendering: pixelated` 로 확대 보간을 끈다. 히어로는 제목 + 사이트 소개(`.site-intro`)만 두고, 인덱스에서 리브의 표기는 "리브"(풀네임 아님)다. 사이드바는 sticky 가 아니며(스크롤을 따라가지 않는다), 920px 이하에서는 `order: -1` 로 목록 위에 올라간다. `a.keeper` 의 hover·focus 스타일은 `a.card` 규칙(제목에 액센트 + 밑줄)과 같은 결로 맞춘다(TASK-8). 마커 구간은 머신 로컬 잡(`~/jobs/docs-ai-summary/`, repo 밖)이 통째로 덮어쓰므로 그 안에 손으로 마크업을 넣지 않는다 — 라벨·문체를 바꾸려면 잡의 `inject.py`·`run.sh` 를 함께 고쳐야 한다(TASK-9). 마커 이름 `AI-SUMMARY` 는 `inject.py` 의 정규식과 짝이라 유지한다. 코멘트 아래에는 월별 목차(`.toc`) 블록이 있으며, 행은 JS 가 갤러리 카드에서 월별 건수를 세어 `/p/archive/`(전체 목록 페이지) 앵커로 연결해 자동 생성한다 — 손으로 갱신하지 않는다(AI-SUMMARY 마커 밖이라 요약 잡과도 무관). 피드는 기본 뷰에서 최근 10건만 풀 카드로 보이고(`RECENT_FULL`, `?recent=N` 오버라이드), 검색·주제 칩이 걸리면 컷을 풀고 전체 카드에서 매치를 보여준다.

### 2-3. 리브 스티커 (m-6, TASK-58\~60)

아바타(28px 얼굴, `.liv` 전용, **정서 축**)와 삽화(폭 1440px 대형, **장면 축**) 사이의 중간 자산. 축은 **동작·상황**이다.

**현역 세트는 파스텔 18종 `backlog/assets/liv/stickers-pastel/` 하나다**(TASK-65). 공식 화풍 기반에 파스텔 블루 복장, 머리카락 광택, 크롭은 상반신 16·전신 1·파묻힘 1, 복장 배분은 OL 7·후디 11 이다.

- **장식 기호를 넣지 않는다.** 반짝임·하트·음표·땀방울을 흩뿌리면 128px 로 줄였을 때 정체불명의 얼룩으로 보인다(TASK-65 에서 실물을 보고 전면 삭제). 기호는 포즈가 그것을 요구하는 컷에서만 **딱 하나 크게** 쓴다 — `idea-bulb` 의 전구, `tilt-question` 의 물음표, `warning-sign` 의 팻말 속 느낌표가 전부다.
- 구 세트 둘은 `backlog/assets/liv/legacy/` 아래로 내렸다(`stickers-standing` = 전신 스탠딩·매끈한 마스코트, `stickers-doodle` = 낙서풍). 신규 페이지에 쓰지 않고 공개 페이지에도 노출하지 않는다. 슬러그가 셋 다 같아 되살릴 일이 생기면 그대로 갈아끼울 수 있다.
- 후처리 스크립트는 세트 공용으로 `backlog/assets/liv/diecut.py` 에 있다.
- 슬러그·쓰임새 표와 규격 근거는 doc-3 §4-6, 파스텔 세트의 스펙과 생성 함정은 §4-8·§4-9 가 정본이다.

- **용처**: 문단이 판정·비교·경고·재작업 같은 태도를 드러내는 문단에 얹는다. 아바타가 이미 담당하는 정서(만족·계면쩍음·하품 등)를 스티커로 되풀이하지 않는다 — 스티커는 표정이 아니라 **동작**으로 상황을 가리킨다.
...
- **크기는 `height` 로 잡는다(`width` 아님).** 스티커는 정사각이 아니라 종횡비가 제각각이라 `width: 104px` 로 주면 높이가 256px 까지 치솟는다. `height: 132px` 가 기본이다. 낙서풍 세트는 크롭이 조여져 같은 높이에서 캐릭터가 구 세트보다 커 보이므로, 문단을 압도하면 그 컷만 높이를 내린다.
...
- **신규 스티커 생성**은 doc-3 §4-6 의 프롬프트 제약(바닥 그림자 금지, 순백 소품 금지, 프레임 여백)과 후처리 스크립트 `backlog/assets/liv/diecut.py` 를 쓴다. **검수는 반드시 실제 표시 크기(128px)에서 한다** — 1024px 원본에서 멀쩡해 보이는 컷이 줄이면 무너진다(18종 중 4종이 그랬다).
...
- **페이지 공통 요소 재생성**: `python3 backlog/assets/archive-thumbs.py` 를 먼저 돌려 대표 이미지(썸네일·OG)를 굽고, 이어서 `python3 backlog/assets/relink-pages.py` 로 각 페이지 head 의 `og:image`(전 카드)와 아티클 하단의 이전/다음·주제 역링크를 다시 만들고 **바뀐 파일을 전부 같은 커밋에 담는다**(TASK-98·TASK-100). 새 글을 발행하면 직전 글의 '다음' 링크도 바뀌므로 **매번 두 파일 이상이 변경된다** — 새 페이지만 커밋하면 직전 글이 최신인 채로 남는다. **순서가 중요하다**: `relink-pages.py` 를 먼저 돌리면 새 글이 공통 폴백 이미지를 가리킨 채로 굳는다. 진실원본은 루트 카드이고, 페이지의 `<!-- PAGEOG:START -->`·`<!-- PAGENAV-CSS:START -->`·`<!-- PAGENAV:START -->` 마커 구간은 스크립트가 통째로 덮어쓰므로 손으로 고치지 않는다(마커 밖은 건드리지 않아 재실행이 멱등이다. 예외로 `twitter:card` 값만 마커 밖에서 `summary_large_image` 로 올린다). `og:image` 는 카드가 있는 페이지 전부에, 이전/다음 내비는 날짜 아티클에만 붙는다 — 갤러리 카드가 없는 페이지는 순서에 낄 수 없어 건너뛴다.
  - `relink-pages.py` 는 아티클 h2 에 **절 앵커와 목차**도 붙인다(TASK-108). id 는 한글을 그대로 쓴 슬러그(`id="그래프가-값을-내는-조건"`)이고 앞머리 번호(`3. `·`<span class="n">3</span>`)는 뺀다 — 번호를 넣으면 절을 하나 끼워 넣는 순간 아래 모든 id 가 밀려 기존 링크가 통째로 깨진다. 같은 제목이 겹치면 `-2`·`-3` 을 붙인다. 스크립트가 붙인 id 에는 `data-anchor="auto"` 가 있어 매번 제목에서 다시 계산되고, 그 표시가 없는 **손으로 붙인 기존 id 는 건드리지 않는다**(그 값으로 나간 링크가 있다). 목차(`<!-- PAGETOC:START -->`, 첫 h2 앞)는 h2 가 5개 이상인 글에만 넣고 미달이면 걷어낸다. 앵커·목차 CSS 는 `<!-- PAGETOC-CSS:START -->` 구간이다. **h2 태그를 고치는 것은 이 스크립트뿐**이므로 h2 안에 `<a class="h-anchor">` 를 손으로 넣지 않는다.
  - `relink-pages.py` 는 끝에 **페이지 용량 경고**를 찍는다(TASK-99). 5MB 를 넘는 페이지가 있으면 stderr 로 알린다 — 경고일 뿐 발행을 막지 않는다. 용량 절약이 목적이 아니라(용량 최적화는 하지 않기로 했고 base64 인라인도 그대로다) **압축을 건너뛴 원본 임베드·같은 이미지 중복 임베드 같은 사고를 발행 전에 알아차리는 것**이 목적이라, 정상 발행이 절대 안 걸리는 값으로 임계를 뒀다(현재 최대 1.24MB). 브리핑의 500KB 상한(TASK-95)과는 무관하다.
...
- **커밋 전에 `python3 backlog/assets/site-check.py` 를 돌린다**(TASK-109). 재생성(§4)이 끝난 뒤 커밋 직전에 돌린다. 사이트 전체를 보고 canonical 존재·값 일치, 내부 링크 깨짐과 앵커 대상, 404 리다이렉트 맵 등록, HTML 용량 상한(5MB)을 확인한다. **위반이 있으면 비영 종료코드로 끝나므로 무인 워처 발행에서도 걸린다** — 고치고 다시 돌린 뒤에 커밋한다. 외부 링크 생존 확인은 느려서 기본 실행에서 빠져 있고 `--external` 로만 돈다(남의 서버 사정으로 흔들려 위반이 아니라 경고로만 센다).
  - 스킬의 `til-preflight.sh`(경로·브랜치·clean·슬러그 충돌)와 `til-verify.sh`(새 페이지의 doctype·favicon·og:title·beacon·카드/README 참조 + impeccable detect)는 둘 다 **그 페이지 하나만** 본다. 이쪽은 사이트 전체를 보므로 중복이 아니다.
...
- **커밋/푸시는 에이전트가 판단해서 한다.** 사용자의 명시적 요청이나 퍼블리시 런북 실행을 기다리지 않는다(2026-07-27 정책 변경 — 그전에는 그 둘일 때만 했다). 작업을 끝내 놓고 push 를 물어보느라 멈추지 말 것.
  - **push 하는 조건**: 작업이 완결됐고(구현 → 검증 → 커밋이 끝났고), `python3 backlog/assets/site-check.py` 가 통과하고, `git status` 에 남의 세션 변경이 섞이지 않았다면 `main` 에 push 한다. 자율 드레인(`/loop-backlog`)이나 백로그 태스크 완료도 포함이다.
  - **push 하지 않고 사용자에게 넘기는 경우**: 점검이 실패했거나, 절반만 된 상태이거나, 되돌리기 어려운 결정이 섞였거나(슬러그·URL 구조 변경, 페이지 삭제), 공개해도 되는지 자체가 판단 대상일 때(저작권·초상권·개인정보). 이 저장소는 public 이라 push 하는 순간 공개되고 히스토리에 남는다.
  - push 후에는 무엇을 올렸는지 한 줄로 보고한다. Pages 반영은 몇십 초 뒤다.
