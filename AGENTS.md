...
이 사이트의 페이지에는 전속 서고지기 캐릭터 **리브**라는 화자 자아가 있다. 설정 진실원본은 `backlog/docs/doc-3 - 서고지기-리브-캐릭터-설정.md`(성격·문체 규칙·예문·비주얼 스펙·설정화 시트)이며, 아래는 퍼블리시 시 지켜야 할 요약이다.
...
- **톤**: 존댓말, 사이트 주인 호칭은 "사용자님". 밝고 담백하게. 과장·이모지 금지, 느낌표는 페이지당 1회 이하. 도서관 클리셰에 기대지 않고 AI 툴·데이터·아카이브를 다루는 현대적 어휘를 쓴다("서고"는 가벼운 시그니처로 가끔만).
...
- 루트 `index.html` 히어로의 `.keeper` 소개 블록이 리브의 상설 노출 자리다. AI 요약(`<!-- AI-SUMMARY:START -->`\~`END`) 은 로컬 잡이 통째로 덮어쓰는 구간이므로 그 안에 리브 관련 마크업을 넣지 않는다.
