...
**현역 세트는 둘이고 슬러그가 같아 통째로 갈아끼울 수 있다.** 복장 배분(OL 7·후디 11)도 동일하다.

| 세트 | 경로 | 화풍 | 크롭 |
|---|---|---|---|
| 낙서풍 | `backlog/assets/liv/stickers-doodle/` | 떨리는 잉크 선, 힘 빠진 실루엣 | 상반신 13·허벅지 3·전신 1·파묻힘 1 |
| 파스텔 | `backlog/assets/liv/stickers-pastel/` | 공식 화풍 기반, 파스텔 블루 복장, 머리카락 광택, 만화 기호 | 상반신 16·전신 1·파묻힘 1 |

- **고르는 기준은 글의 톤이다.** 가볍고 소개·안내·입문 성격의 글은 파스텔, 자조·실패·재작업 고백·함정 경고가 있는 글은 낙서풍. **한 페이지에서는 한 세트로 고정한다**(두 화풍이 섞이면 어색하다). 애매하면 낙서풍.
- 구 세트(전신 스탠딩·매끈한 마스코트)는 `backlog/assets/liv/legacy/stickers-standing/` 으로 내렸다. 신규 페이지에 쓰지 않는다.
- 후처리 스크립트는 세트 공용으로 `backlog/assets/liv/diecut.py` 에 있다.
- 슬러그·쓰임새 표와 규격 근거는 doc-3 §4-6, 낙서풍의 차이는 §4-7, 파스텔의 차이와 생성 함정은 §4-8·§4-9 가 정본이다.
