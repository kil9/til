...
- 루트 `index.html` 히어로의 `.keeper` 소개 블록이 리브의 상설 노출 자리다. 아바타·이름·한 줄 소개를 감싼 `a.keeper` 전체가 리브 소개 페이지(`/liv-today/`)로 가는 링크이며, hover·focus 스타일은 `a.card` 규칙(제목에 액센트 + 밑줄)과 같은 결로 맞춘다(TASK-8). AI 요약(`<!-- AI-SUMMARY:START -->`\~`END`) 은 로컬 잡이 통째로 덮어쓰는 구간이므로 그 안에 리브 관련 마크업을 넣지 않는다.
