...
- 최신 피드 + 전체 색인(2026-07-21, 구 이번달-축약 연/월 아카이브 대체): 최근 10건만 풀 카드로 두고, 아래 전체 색인이 모든 글(피드 노출분 포함)을 월별 그룹 + 월 점프 링크(건수 표기)로 나열한다. `?recent=N` 오버라이드(검증용). `/til-archive/` 카드는 구 til repo 마지막 업데이트 `2026-04-02` 기준이라 색인 하단에 위치.
