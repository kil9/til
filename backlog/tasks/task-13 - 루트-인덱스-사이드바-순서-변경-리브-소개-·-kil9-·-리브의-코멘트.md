...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 루트 index.html 의 aside.side 가 리브 소개(a.keeper) → kil9(.human) → 리브의 코멘트(.ai-summary) 순으로 렌더링된다
- [ ] #2 AI-SUMMARY:START/END 마커 쌍이 그대로 유지되고, 마커 안 마크업은 변경되지 않는다
- [ ] #3 구분선(border-top)이 .ai-summary 앞에만 있고, 리브 소개와 kil9 블록 사이에는 여백만 있다
- [ ] #4 a.keeper 가 최상단이 되면서 어색해진 margin-top/padding-top 이 정리돼 있다
- [ ] #5 920px 이하 반응형(사이드바가 order:-1 로 목록 위)에서도 세 블록 순서·여백이 깨지지 않는다
- [ ] #6 AGENTS.md 의 사이드바 구조 설명이 새 순서와 일치한다
<!-- AC:END -->
