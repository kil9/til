...
doc-3(리브 캐릭터 설정)을 헤드리스 주입용 시스템 프롬프트로 요약. 대화 응대용(간결·나른·존댓말·사용자님 호칭)과 TIL 근거 Q&A 규약 포함.
...
대화 레지스터는 Fable advisor(chihaya) 자문 반영 — TIL 페이지 보고서체와 구분되는 실시간 Slack 대화체: 바로 본론·1-3문장 기본·캐릭터성 4-5턴당 1회 이하·호칭 사용자님은 사이트 주인 한정·한숨/말줄임표 연기 및 설정 낭독 금지. 안티패턴(나른함 과잉·설정 낭독·보고서체 유입·이모지 남발·과잉 사과·도서관 어휘 회귀)도 프롬프트에 명시.
